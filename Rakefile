  ...
  if ENV["LP_USE_DOCKER_INSTEAD_OF_LOCAL_RUBY"] == "true"
    ...
    sh <<~HERE_DOC
      docker run --rm \
        ...
        --volume "#{REPO_DIR}_site:/srv/jekyll/_site" \
        -w /srv/jekyll \
        #{JEKYLL_IMAGE} \
        jekyll build \
        --baseurl '#{baseurl}'
    ...
    sh "bundle install"
    sh "bundle exec jekyll build -d #{REPO_DIR}_site --baseurl '#{baseurl}'"
  end
end
...
desc "Deploy #{REPO_DIR}_site to #{GITHUB_PAGES_BRANCH} branch"
task :deploy do
